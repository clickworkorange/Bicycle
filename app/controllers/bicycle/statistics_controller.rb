class Bicycle::StatisticsController < Bicycle::ApplicationController
  def index
    ...
    @most_visited = Ahoy::Event.where(name: "Page view", time: @range).joins(:page).group(:page_id, :title, :template).select(:title, :template, "COUNT(ahoy_events.id) AS count").order('count desc').limit(10)
    @errors = Ahoy::Event.where(name: "Error", time: @range).group(:properties).select(:properties, "COUNT(ahoy_events.id) AS count").order('count desc').limit(10)
    @referrers = Ahoy::Visit.group(:referrer).where.not(referrer: nil).select(:referrer, "COUNT(ahoy_visits.id) AS count").order('count desc').limit(10)
    # Note: Ahoy::Visit.location is a virtual column created from Ahoy::Visit.country, region and city
    @locations = Ahoy::Visit.group(:location).where.not(location: nil).select(:location, "COUNT(ahoy_visits.id) AS count").order('count desc').limit(10)
